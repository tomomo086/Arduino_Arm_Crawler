...
### プロジェクト説明
タミヤのアームクローラー工作キットをベースに、Arduino Uno R4 WiFiとAndroidアプリを使用してBLE（Bluetooth Low Energy）でワイヤレス制御するIoTシステムを開発。**AI駆動開発手法**を段階的に活用し、初期開発ではGrok 3、Claude 4、Cursorを使用し、Streamlit対応時にはClaudeCodeを活用した開発プロセスを実践。入門的なロボット制御システムを構築。
...
### AI駆動開発の実践
- **初期開発**: Grok 3、Claude 4、Cursorを活用
...
AI駆動開発: 初期開発（Grok 3 + Claude 4 + Cursor）→ Streamlit対応（ClaudeCode）

*本プロジェクトは、段階的なAI駆動開発の実践として、初期開発ではGrok 3、Claude 4、Cursorを活用し、Streamlit対応時にはClaudeCodeを使用した開発プロセスを実践した教育・学習目的のオープンソースプロジェクトです。*